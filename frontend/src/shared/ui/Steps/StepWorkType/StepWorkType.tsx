...
export const StepWorkType = ({
  ...
  formData: { workType: string };
  onFormDataChange: (data: Partial<{ workType: string }>) => void;
  options: string[];
}) => {
  const value = formData.workType || "";
  return (
    <Box>
      ...
      <Box display="flex" alignItems="center" gap={2} mb={2}>
        ...
        <Box flex={1}>
          <TextField
            ...
            label="Тип работ"
            value={value || ''}
            onChange={(e) => onFormDataChange({ workType: e.target.value })}
            fullWidth
