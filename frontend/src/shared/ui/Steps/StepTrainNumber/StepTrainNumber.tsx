...
export const StepTrainNumber = ({
  ...
  formData: { trainNumber: string };
  onFormDataChange: (data: Partial<{ trainNumber: string }>) => void;
  options: string[];
}) => {
  const value = formData.trainNumber || '';
  return (
    <Box>
      ...
      <Box display="flex" alignItems="center" gap={2} mb={2}>
        ...
        <Box flex={1}>
          <TextField
            ...
            label="Номер поезда"
            value={value || ''}
            onChange={(e) => onFormDataChange({trainNumber: e.target.value})}
            fullWidth
            variant="outlined"
          ...
          >
            {options.map((number, index) => (
              <MenuItem
                key={`${number}-${index}`}
                value={number}
                className="step-train-number-menu-item"
              >
                {number}
              </MenuItem>
            ))}
