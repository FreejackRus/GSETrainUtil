...
import { referenceApi } from '../../../shared';
import { useUser } from '../../../shared/contexts/UserContext';
import { getFile } from '../../../shared/api/uploadApi';
    ...
    try {
      ...
      if (draft) {
        ...
        for (const c of draft.carriages || []) {
          ...
          const mappedEquip: EquipmentFormItem[] = await Promise.all(
            (c.equipment || []).map(async (e) => {
              const equipmentPhotoPath = e.photos.find((item) => item.type === 'equipment')?.path;
              const serialPhotoPath = e.photos.find((item) => item.type === 'serial')?.path;
              const macPhotoPath = e.photos.find((item) => item.type === 'mac')?.path;
              ...
              return {
                ...
                photos: {
                  equipment: equipmentPhotoPath ? await getFile(equipmentPhotoPath) : null,
                  serial: serialPhotoPath ? await getFile(serialPhotoPath) : null,
                  mac: macPhotoPath ? await getFile(macPhotoPath) : null,
                },
              };
            }),
          );
          arr.push({
            ...
            carriageType: c.type || '',
            carriagePhotos: {
              carriage: c.photo ? await getFile(c.photo) : null,
              equipment: c.generalPhotoEquipmentCarriage
                ? await getFile(c.generalPhotoEquipmentCarriage)
                : null,
            }, // файлы не подтягиваем
