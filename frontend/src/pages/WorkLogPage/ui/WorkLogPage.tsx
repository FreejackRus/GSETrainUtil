...
export const WorkLogPage = () => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(entry => {
        ...
        if (entry.macAddress && entry.equipment) {
          ...
          if ((equipmentType.includes('точка доступа') || 
  ...
  const getWorkTypeColor = (workType: string): 'primary' | 'secondary' | 'success' | 'warning' | 'error' | 'info' | 'default' => {
    if (!workType) return 'default';
    switch (workType.toLowerCase()) {
      case 'установка': return 'success';
      case 'замена': return 'warning';
      case 'ремонт': return 'error';
      case 'диагностика': return 'info';
      case 'настройка': return 'primary';
    ...
    <Container maxWidth="xl" className="work-log-page">
      <Box>
        ...
        {filteredEntries.length === 0 ? (
          ...
          <Box>
            {filteredEntries.map(renderWorkLogList)}
          </Box>
        )}
        ...
        >
          <DialogTitle>
            Фотографии заявки #{selectedEntry?.applicationNumber}
          </DialogTitle>
          <DialogContent>
            {selectedEntry && (
