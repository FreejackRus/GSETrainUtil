...
export const createTechnicalActAcceptance = async (
  ...
  equipmentTypes.map((item, index) => {
    arrEquipment.push({
      name: item,
      serial: serialNumbers[index],
      note: String(countEquipments[index]) || "-",
    });
  });
  const resultJson: Act = {
    actNumber: String(applicationNumber),
    contractDate: contractDate ?? "«15» июля 2025 г.",
  ...
  const filePath = path.resolve(outputDir, `${trainNumber} от ${formatRussianDate(applicationDate)}pdf`);
