...
function createInstallationingAct(doc: jsPDF, data: InstallationingActData): number {
  ...
  doc.text("АКТ монтажа оборудования", pageWidth / 2, 30, {
    align: "center",
  });
...
export const createPdfActInstEquipment = async (
  json: ResponseJson,
  outputDir: string
) => {
  const doc = new jsPDF();
  ...
  createInstallationingAct(doc, resultJson);
  ...
  const fileName = `Акт монтажа №${String(applicationNumber).trim() || "unknown"}.pdf`;
