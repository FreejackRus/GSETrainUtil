...
import { createWordActDisEquipment } from "../../wordGenerate/generate/actDisEquipment";

export const postWordActDisEquipment = async (req: Request, res: Response) => {
  const body = req.body;
  console.log(body);
   try {
       const WORD_DIR = resolveWordDir();
       const fileName = `Акт Демонтажа №${String(body.applicationNumber).trim() || "unknown"}.docx`;

       // генерим прямо в WORD_DIR, генератор вернёт полный путь
       const filePath = await createWordActDisEquipment(body, WORD_DIR);
