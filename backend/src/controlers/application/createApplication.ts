...
const ALLOWED_EQUIP_PHOTO_KEYS: EquipmentPhotoType[] = ["equipment", "serial", "mac"];
...
async function generateUniqueFilename(
  ...
  while (true) {
    ...
    if (scope === "carriage") {
      const exists = await prisma.requestCarriagePhoto.findFirst({ where: { photoPath: relPath } });
      if (exists) continue;
    } else {
  ...
  try {
    ...
    if (isCompleted) {
      if (
          ...
          trains.every((t) => t.carriages.every((c) => c.equipments.length === 0)) ||
          !performer ||
          !currentLocation
      ) {
        return res.status(400).json({
      ...
      for (const t of trains) {
        if (!t.trainNumber) {
          return res.status(400).json({ success: false, message: "У каждого поезда обязателен trainNumber" });
        }
        for (const c of t.carriages) {
          if (!c.carriageNumber || !c.carriageType) {
            return res.status(400).json({
              success: false,
              message: "В каждом вагоне обязательны carriageNumber и carriageType",
            });
          }
        }
      }
    ...
    for (const t of trains) {
      ...
      for (const c of t.carriages) {
        ...
        for (const e of c.equipments) {
          ...
          let eqRec = null;
          if (e.serialNumber) {
            eqRec = await prisma.equipment.findUnique({ where: { serialNumber: e.serialNumber } });
          }
          if (!eqRec && e.macAddress) {
            eqRec = await prisma.equipment.findUnique({ where: { macAddress: e.macAddress } });
          }
          if (!eqRec) {
